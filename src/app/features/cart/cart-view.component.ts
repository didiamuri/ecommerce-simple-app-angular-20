    ...
    <div class="bg-white">
      <div class="max-w-2xl mx-auto pt-16 pb-24 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <div *ngIf="cartService.cartItems().length === 0" class="mt-12 text-center">
            ...
            <a routerLink="/products" class="mt-4 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200">
                Continue Shopping
            </a>
        </div>

        <form *ngIf="cartService.cartItems().length > 0" class="mt-12 lg:grid lg:grid-cols-12 lg:gap-x-12 lg:items-start xl:gap-x-16">
            ...
            <ul role="list" class="border-t border-b border-gray-200 divide-y divide-gray-200">
              <li *ngFor="let item of cartService.cartItems()" class="flex py-6 sm:py-10">
                <div class="flex-shrink-0">
                  <img [src]="item.thumbnail" [alt]="item.title" class="w-24 h-24 rounded-md object-center object-cover sm:w-48 sm:h-48">
                </div>
                  ...
                  <div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                    <div>
                      <div class="flex justify-between">
                        <h3 class="text-sm">
                          <a [routerLink]="['/products', item.id]" class="font-medium text-gray-700 hover:text-gray-800">
                            {{ item.title }}
                          </a>
                        </h3>
                      </div>
                    ...
                    <div class="mt-4 sm:mt-0 sm:pr-9">
                      ...
                      <select [id]="'quantity-' + item.id" [name]="'quantity-' + item.id" [value]="item.quantity" 
                      ...
                      <div class="absolute top-0 right-0">
                        <button type="button" (click)="cartService.removeFromCart(item.id)" class="-m-2 p-2 inline-flex text-gray-400 hover:text-gray-500">
          ...
          <section aria-labelledby="summary-heading" class="mt-16 bg-gray-50 rounded-lg px-4 py-6 sm:p-6 lg:p-8 lg:mt-0 lg:col-span-5">
            ...
            <dl class="mt-6 space-y-4">
              <div class="flex items-center justify-between">
                ...
                <dd class="text-sm font-medium text-gray-900">\${{ cartService.totalPrice() | number:'1.2-2' }}</dd>
              </div>
              <div class="border-t border-gray-200 pt-4 flex items-center justify-between">
                <dt class="text-base font-medium text-gray-900">Order total</dt>
                <dd class="text-base font-medium text-gray-900">\${{ cartService.totalPrice() | number:'1.2-2' }}</dd>
              </div>
            </dl>
